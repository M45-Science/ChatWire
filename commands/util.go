		...
		for _, c := range cl {
			...
			if strings.EqualFold(c.AppCmd.Name, data.Name) {
				...
				if c.AdminOnly {
					...
					cwlog.DoLogAudit("%s: command: %s", i.Member.User.Username, data.Name)
					runCommand(&c, i)
					return
				}
			}
...
var playerPerms int64 = discordgo.PermissionUseApplicationCommands //Use application commands
